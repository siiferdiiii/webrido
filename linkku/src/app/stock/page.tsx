...
export default function StockPage() {
  ...
  return (
    <>
      <Topbar title="Kelola Data Produk" subtitle="Manajemen stok akun yang terikat pada pelanggan" />
      ...
      <div className="px-4 md:px-8 pb-8 space-y-5">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-lg font-bold text-white">Filter Produk</h2>
              <p className="text-xs text-[var(--text-muted)]">Klik produk untuk mengelola data akun khusus produk tersebut</p>
            </div>
          </div>
        ...
        {selectedProduct && (
          <div className="flex items-center gap-3 px-4 py-3 rounded-xl" style={{ background: `${selectedProduct.type === "mobile" ? "#22c55e" : "#3b82f6"}10`, border: `1px solid ${selectedProduct.type === "mobile" ? "#22c55e" : "#3b82f6"}30` }}>
            <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: `${selectedProduct.type === "mobile" ? "#22c55e" : "#3b82f6"}20` }}>
              {selectedProduct.type === "mobile" ? <Smartphone size={16} style={{ color: "#22c55e" }} /> : <Monitor size={16} style={{ color: "#3b82f6" }} />}
            </div>
            <div className="flex-1">
              <p className="text-sm font-semibold text-white">Stok Akun: {selectedProduct.name}</p>
              <p className="text-[10px] text-[var(--text-muted)]">SKU {selectedProduct.id} · {formatCurrency(selectedProduct.price)} · {selectedProduct.duration} hari</p>
            </div>
            <button onClick={() => { setSelectedProduct(null); setProductTypeFilter("Semua"); }} className="btn-icon" title="Lihat semua"><X size={16} /></button>
          </div>
        )}
